...
def search_all_case_data_files(filepath: str | None = None) -> list:
    ...
    """
    case_data_filepath = (
        os.path.join(httpseeker_path.case_data_dir, f'{httpseeker_config.PROJECT_NAME}') if filepath is None else filepath
    )
    files = (
...
def search_all_testcase_files() -> list:
    ...
    """
    files = glob.glob(
        os.path.join(httpseeker_path.testcase_dir, f'{httpseeker_config.PROJECT_NAME}', '**', 'test_*.py'), recursive=True
    )
    return files
