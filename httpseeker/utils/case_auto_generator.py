...
from httpseeker.utils.file_control import get_file_property, search_all_case_data_files, search_all_testcase_files
...
'''
                # 创建测试用例文件
                tag = case_filename.split(httpseeker_config.PROJECT_NAME)[1].split(os.path.sep)[1:-1]
                new_testcase_filename = testcase_func_name + '.py'
                if tag:
                    case_path = os.path.join(
                        httpseeker_path.testcase_dir, httpseeker_config.PROJECT_NAME, *tag, new_testcase_filename
                    )
                else:
                    case_path = os.path.join(
                        httpseeker_path.testcase_dir, httpseeker_config.PROJECT_NAME, new_testcase_filename
                    )
