...
# 指定环境和配置文件（支持相对路径）
python httpseeker/cli.py --env test.env --conf_toml httpseeker/core/conf_toml.toml --auth httpseeker/core/auth.yaml --run
...
```bash
# 指定环境变量文件
python httpseeker/cli.py --env test.env --run
