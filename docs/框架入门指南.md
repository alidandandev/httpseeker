...
python cli.py \
  ...
  --conf_toml core/conf_toml/like_bofa_h5.toml \
  --auth core/auth_yaml/like_bofa_h5.yaml \
  --run
...
python httpseeker/cli.py --auth httpseeker/core/like_bofa_h5.yaml --run

# 同时指定多个配置（推荐格式）
python httpseeker/cli.py --env like_bofa_h5_test.env --conf_toml httpseeker/core/conf_toml.toml --auth httpseeker/core/like_bofa_h5.yaml --run
```
...
3. **多项目配置**：不同项目使用独立的配置文件
   ...
   # 使用绝对路径
   python httpseeker/cli.py \
     --conf_toml /projects/project_a/conf_toml/like_bofa_h5.toml \
     --auth /projects/project_a/auth_yaml/like_bofa_h5.yaml \
     --run
   ```
...
run(
    ...
    auth_path='/path/to/like_bofa_h5.yaml'    # 支持绝对路径和相对路径
)
```
...
3. **验证配置**
   ...
   ```bash
   # 检查文件是否存在
   ls -l httpseeker/core/auth_yaml/like_bofa_h5.yaml
   ls -l httpseeker/core/conf_toml/like_bofa_h5.toml
   ```
