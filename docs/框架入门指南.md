...
# 简化写法（从 httpseeker 目录内运行）
cd httpseeker
python cli.py \
  --env core/run_env/Dz_like_bofa_h5.env \
  --conf_toml core/conf_toml/Dz_like_bofa_h5.toml \
  --auth core/auth_yaml/Dz_like_bofa_h5.yaml \
  --run

# 使用不同环境
python httpseeker/cli.py \
  --env httpseeker/core/run_env/prod.env \
  --conf_toml httpseeker/core/conf_toml/Dz_like_bofa_h5.toml \
  --auth httpseeker/core/auth_yaml/Dz_like_bofa_h5.yaml \
  --run

# 查看 CLI 帮助信息（已全面中文化）
python httpseeker/cli.py --help

# 使用 pytest 直接运行（需要先清理缓存）
python -B -m pytest testcases/

# 运行特定项目的测试
python -B -m pytest testcases/your_project/

# 运行特定测试文件
python -B -m pytest testcases/your_project/test_login.py

# 生成 allure 报告
pytest testcases/ --alluredir=./report/allure_report
...
config:
  ...
  # 请求配置
  request:
    env: "Dz_like_bofa_h5.env"          # 环境配置文件
    headers:                 # 全局请求头
      Content-Type: "application/json"
    timeout: 30              # 超时时间（秒）
    verify: false            # SSL 验证
...
test_steps:
  ...
  - name: "测试用例3"
    case_id: "test_003"
    is_run:               # 条件执行
      env: "Dz_like_bofa_h5.env"     # 仅在 test 环境执行
    request:
      # ...
```
...
# 测试环境
httpseeker/core/run_env/Dz_like_bofa_h5.env
...
config:
  request:
    env: "Dz_like_bofa_h5.env"  # 指定使用测试环境
```
...
```bash
# 指定环境变量文件
python httpseeker/cli.py --env Dz_like_bofa_h5.env --run

# 指定自定义配置文件（支持相对路径和绝对路径）
python httpseeker/cli.py --conf_toml httpseeker/core/conf_toml.toml --run

# 指定自定义认证文件（支持相对路径和绝对路径）
python httpseeker/cli.py --auth httpseeker/core/Dz_like_bofa_h5.yaml --run

# 同时指定多个配置（推荐格式）
python httpseeker/cli.py --env Dz_like_bofa_h5.env --conf_toml httpseeker/core/conf_toml.toml --auth httpseeker/core/Dz_like_bofa_h5.yaml --run
```
...
| 参数 | 类型 | 说明 | 示例 |
|------|------|------|------|
| `-V, --version` | 选项 | 打印版本信息 | `python httpseeker/cli.py --version` |
| `-r, --run` | 选项 | 运行测试用例，支持自定义 pytest 参数 | `python httpseeker/cli.py --run` |
| `--env <环境文件>` | 参数 | 指定环境变量文件路径（完整路径或相对路径） | `--env httpseeker/core/run_env/prod.env` |
| `--conf_toml <配置文件>` | 参数 | 指定配置文件路径，**支持相对路径和绝对路径** | `--conf_toml httpseeker/core/conf_toml/conf.toml` |
| `--auth <认证文件>` | 参数 | 指定认证配置文件路径，**支持相对路径和绝对路径** | `--auth httpseeker/core/auth_yaml/auth.yaml` |

**⚠️ 重要提示：参数顺序**
- `--env`、`--conf`、`--auth` 必须放在 `--run` 之前
- 正确示例：`python httpseeker/cli.py --env test.env --run`
- 错误示例：`python httpseeker/cli.py --run --env test.env` ❌
...
1. **多环境测试**：不同环境使用不同的 env 文件
   ...
   # 开发环境
   python httpseeker/cli.py \
     --env httpseeker/core/run_env/dev.env \
     --conf_toml httpseeker/core/conf_toml/Dz_like_bofa_h5.toml \
     --auth httpseeker/core/auth_yaml/Dz_like_bofa_h5.yaml \
     --run
   ...
   # 生产环境
   python httpseeker/cli.py \
     --env httpseeker/core/run_env/pro.env \
     --conf_toml httpseeker/core/conf_toml/Dz_like_bofa_h5.toml \
     --auth httpseeker/core/auth_yaml/Dz_like_bofa_h5.yaml \
     --run
   ```

2. **完整配置运行**（推荐）：明确指定所有配置文件
   ```bash
   # 在项目根目录执行（使用相对路径）
   python httpseeker/cli.py \
     --env httpseeker/core/run_env/Dz_like_bofa_h5.env \
...
3. **多项目配置**：不同项目使用独立的配置文件
   ```bash
   # 使用绝对路径
   python httpseeker/cli.py \
     --conf_toml /projects/project_a/conf_toml/Dz_like_bofa_h5.toml \
     --auth /projects/project_a/auth_yaml/Dz_like_bofa_h5.yaml \
     --run
   ```

4. **CI/CD 集成**：在持续集成中动态指定配置
   ```bash
   python httpseeker/cli.py \
     --env ${ENV_FILE} \
     --conf_toml ${CONF_PATH} \
...
5. **查看帮助信息**：查看完整的中文帮助
   ...
   # 子命令帮助
   python httpseeker/cli.py testcase --help
   python httpseeker/cli.py import --help
   ```
...
run(
    global_env='prod.env',
    conf_path='/path/to/conf_toml.toml',  # 支持绝对路径和相对路径
    auth_path='/path/to/Dz_like_bofa_h5.yaml'    # 支持绝对路径和相对路径
)
```
...
**框架新特性（v0.8.1+）：**
- ✅ **CLI 相对路径支持**：`--conf` 和 `--auth` 参数现在支持相对路径，无需写完整的绝对路径
- ✅ **中文化 CLI 帮助**：所有 CLI 命令帮助信息已全面中文化，使用更友好
- ✅ **修复 Hooks 跨环境问题**：修复了自定义 hooks 函数在不同环境下执行失败的问题
...
### 1. FileNotFoundError: auth.yaml 或 conf.toml 找不到？

**问题描述：**
```
FileNotFoundError: [Errno 2] No such file or directory: '/path/to/httpseeker/core/auth.yaml'
```

**原因分析：**
框架在版本更新中调整了配置文件目录结构：
- `auth.yaml` 移动到 `httpseeker/core/auth_yaml/` 目录
- `conf.toml` 移动到 `httpseeker/core/conf_toml/` 目录
...
# 清理pytest缓存
rm -rf .pytest_cache httpseeker/.pytest_cache

# 使用 -B 参数运行（不生成字节码）
python -B -m pytest testcases/
```

**方法3: 检查path_conf.py配置**

确保 `httpseeker/core/path_conf.py` 中的配置正确：
```python
@property
def auth_conf_dir(self) -> str:
    """AUTH配置文件路径"""
    return os.path.join(self.core_dir, 'auth_yaml')  # 确保是 'auth_yaml'
```
...
**变更内容：**
- `auth.yaml` 从 `httpseeker/core/` 移动到 `httpseeker/core/auth_yaml/`
- `conf.toml` 从 `httpseeker/core/` 移动到 `httpseeker/core/conf_toml/`
...
3. **验证配置**
   ...
   ```bash
   # 检查文件是否存在
   ls -l httpseeker/core/auth_yaml/Dz_like_bofa_h5.yaml
   ls -l httpseeker/core/conf_toml/Dz_like_bofa_h5.toml
   ```
...
```
FileNotFoundError: [Errno 2] No such file or directory: '.../httpseeker/core/auth.yaml'
```
**解决：** 使用正确的路径 `httpseeker/core/auth_yaml/auth.yaml`

❌ **错误2：导入错误（缓存问题）**
```
ERROR collecting testcases/...
```
...
```bash
# 运行测试收集，应该能成功收集所有用例
python -B -m pytest testcases/ --collect-only
```
