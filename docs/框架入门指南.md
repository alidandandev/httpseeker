...
# 使用不同环境
python httpseeker/cli.py --env prod.env --run
...
| `--env <环境文件>` | 参数 | 指定环境变量文件名（仅文件名，如 test.env），会覆盖 conf.toml 中的配置 | `--env prod.env` |
| `--conf <配置文件>` | 参数 | 指定配置文件路径，**支持相对路径和绝对路径**，默认使用 httpseeker/core/conf.toml | `--conf httpseeker/core/conf.toml` |
| `--auth <认证文件>` | 参数 | 指定认证配置文件路径，**支持相对路径和绝对路径**，默认使用 httpseeker/core/auth.yaml | `--auth httpseeker/core/auth.yaml` |
...
1. **多环境测试**：不同环境使用不同的 env 文件
   ```bash
   # 开发环境
   python httpseeker/cli.py --env dev.env --run

   # 测试环境
   python httpseeker/cli.py --env like_bofa_h5_test.env --run

   # 生产环境
   python httpseeker/cli.py --env pro.env --run
   ```
...
2. **完整配置运行**（推荐）：明确指定所有配置文件
   ...
   python httpseeker/cli.py \
     --env like_bofa_h5_test.env \
     --conf_toml httpseeker/core/conf_toml.toml \
     --auth httpseeker/core/auth.yaml \
     --run
   ```
...
3. **多项目配置**：不同项目使用独立的配置文件
   ...
   # 使用绝对路径
   python httpseeker/cli.py \
     --conf_toml /projects/project_a/conf_toml.toml \
