...
# 指定环境和配置文件（支持相对路径）
python httpseeker/cli.py --env test.env --conf httpseeker/core/conf.toml --auth httpseeker/core/auth.yaml --run
...
python httpseeker/cli.py --conf httpseeker/core/conf.toml --run
...
2. **完整配置运行**（推荐）：明确指定所有配置文件
   ...
   python httpseeker/cli.py \
     --env test.env \
     --conf httpseeker/core/conf.toml \
...
run(
    global_env='prod.env',
    conf_path='/path/to/conf.toml',  # 支持绝对路径和相对路径
