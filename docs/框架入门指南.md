...
python httpseeker/cli.py \
  ...
  --conf_toml httpseeker/core/conf_toml/conf.toml \
  --auth httpseeker/core/auth_yaml/auth.yaml \
  --run

# 简化写法（从 httpseeker 目录内运行）
cd httpseeker
python cli.py \
  --env core/run_env/like_bofa_h5_test.env \
  --conf_toml core/conf_toml/conf.toml \
  --auth core/auth_yaml/auth.yaml \
  --run
...
config:
  ...
  # 请求配置
  request:
    env: "like_bofa_h5_test.env"          # 环境配置文件
    headers:                 # 全局请求头
      Content-Type: "application/json"
    timeout: 30              # 超时时间（秒）
    verify: false            # SSL 验证
...
test_steps:
  ...
  - name: "测试用例3"
    case_id: "test_003"
    is_run:               # 条件执行
      env: "like_bofa_h5_test.env"     # 仅在 test 环境执行
    request:
      # ...
```
...
# 测试环境
httpseeker/core/run_env/like_bofa_h5_test.env
...
config:
  request:
    env: "like_bofa_h5_test.env"  # 指定使用测试环境
```
...
```bash
# 指定环境变量文件
python httpseeker/cli.py --env like_bofa_h5_test.env --run

# 指定自定义配置文件（支持相对路径和绝对路径）
python httpseeker/cli.py --conf_toml httpseeker/core/conf_toml.toml --run

# 指定自定义认证文件（支持相对路径和绝对路径）
python httpseeker/cli.py --auth httpseeker/core/auth.yaml --run

# 同时指定多个配置（推荐格式）
python httpseeker/cli.py --env like_bofa_h5_test.env --conf_toml httpseeker/core/conf_toml.toml --auth httpseeker/core/auth.yaml --run
```
...
3. **多项目配置**：不同项目使用独立的配置文件
   ...
   # 使用绝对路径
   python httpseeker/cli.py \
     --conf_toml /projects/project_a/conf_toml/conf.toml \
     --auth /projects/project_a/auth_yaml/auth.yaml \
     --run
   ```
...
run(
    ...
    auth_path='/path/to/auth.yaml'    # 支持绝对路径和相对路径
)
```
...
3. **验证配置**
   ...
   ```bash
   # 检查文件是否存在
   ls -l httpseeker/core/auth_yaml/auth.yaml
   ls -l httpseeker/core/conf_toml/conf.toml
   ```
